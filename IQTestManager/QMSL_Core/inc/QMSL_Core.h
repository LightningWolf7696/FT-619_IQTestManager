/******************************************************************************
 * $Header: //depot/HTE/QDART/QMSL6.1/QMSL_Core/inc/QMSL_Core.h#12 $
 * $DateTime: 2019/01/10 21:14:48 $
 *
 *
 ******************************************************************************
 *
...
#define QLIB_EFS_MAX_FILENAME_LEN DIAG_FS_MAX_PATHNAME_LEN
   ...
   *******************************************************************************/
   ...
   /******************************************************************************/
   /**
   Start a TCP/IP server to accept a diag connection over socket from target on QPHONEMS mode. 
   Or Start a TCP/IP client to connect to a remote target running diag over socket on QPHONEMS mode.
   If QMSL starts as TCP/IP server, it serves the similar purpose as QPST's IP server to accept client connection.
   When successful, QMSL_Timeout_IsPhoneConnected will be changed from 80 (ms) to 2000 (ms) since TCP/IP connection has slower latency than USB

   \param iComPort = COM port number to use for communication with phone through TCP/IP. This is a virtual COM port number. User should choose unused COM port number.
          bConnection_direction = Determines QMSL serves as TCP/IP Server or clients. If it is QLIB_CONNECTION_IP_INCOMING, serves as TCP/IP server. If it is QLIB_CONNECTION_IP_OUTGOING, serve as TCP/IP client
          resourceStringLength = the length of resourceString characters. If zero and TCP/IP server case, use default port 2500. 
          resourceString = Specify IP address and port number, also, it can execute additional programs. 
            Case 1) IP address and port number:  "127.0.0.1:25001"
            Case 2) Port number only : "2500"
            Case 3) IP address and port number, and additional program to execute  "127.0.0.1:4321|qcmbr.exe -r blah_blah_blah"
         bCheckConnection = Check connection is established or not. Set 1 by default.
         retryTimeoutSec = Set Timeout value in second order
         errorCode = Return error code. Error code includes 
         typedef enum
         {
            QLIB_TCPIP_CONNECT_SUCCESS = 0,
            QLIB_TCPIP_CONNECT_FAILURE = 1, // General failure
            QLIB_TCPIP_CONNECT_IsPhoneConnected_FAILURE = 2, //
            QLIB_TCPIP_CONNECT_Unsupported_Direction = 3,
            QLIB_TCPIP_CONNECT_Empty_ResourceString = 4,
            QLIB_TCPIP_CONNECT_Unsupported_ResourceString = 5,
            QLIB_TCPIP_CONNECT_Socket_Creation_Failure = 6,
            QLIB_TCPIP_CONNECT_Socket_Creation_Timeout = 7, // No response/request from server/client within timeout
            QLIB_TCPIP_CONNECT_IPaddr_Not_Resolved = 8, // No response/request from server/client within timeout
         } QLIB_TCPIP_CONNECT_ERROR_TYPE;
   \return  HANDLE to use for all subsequent QLIB calls that require a handle.
   ...
   *******************************************************************************/
   QLIB_API HANDLE QLIB_QPHONEMS_ConnectTCPIP(int iComPort, unsigned char bConnection_direction  /* eConnectionDirection {QMSL_CONNECTION_INCOMING, QMSL_CONNECTION_IP_OUTGOING}. */,  
   ...
   *******************************************************************************/
   ...
   Set the library mode--QPST, QPHONEMS or QUTS mode.  
   The default state of the library is to use QPHONEMS.  
   In order to use QPST, this function must be set to QLIB_LIB_MODE_QPST

   \param iLibraryMode = 1 to use QPST, 0 to use QPHONEMS, 2 to use QUTS
   \return void

   *******************************************************************************/
   QLIB_API void QLIB_SetLibraryMode( unsigned char iLibraryMode );
