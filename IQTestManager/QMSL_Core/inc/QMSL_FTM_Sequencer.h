/******************************************************************************
 * $Header: //depot/HTE/QDART/QMSL6.1/QMSL_Core/inc/QMSL_FTM_Sequencer.h#7 $
...
extern "C" {
   ...
   *******************************************************************************/
   ...
   /******************************************************************************/
   /**
         Set the request and response payload type. Payload can be in a compressed
      or uncompressed fashion

      \param iPayLoadType : false = Uncompressed payload; true = Compressed payload

      \return TRUE if this operation is successful

      \warning this function does not send any diag commands to mobile
   *******************************************************************************/
   QLIB_API unsigned char QLIB_FTM_SEQ_SetPayloadType(HANDLE hResourceContext, unsigned char isSeqCompressed);

   /******************************************************************************/
   /**
      Set the response type to get the test results. Results can be filtered from
      asynchronous log packets or from the response obtained by dispatching the 
      API to get results for each command ID enqueued in the sequence.

      \param iResponseType: 0 = get results from Async log packets
                            1 = get results from the sync API

      \return TRUE if this operation is successful

      \warning this function does not send any diag commands to mobile
   *******************************************************************************/
   QLIB_API unsigned char  QLIB_FTM_SEQ_SetResponseType(HANDLE hResourceContext, unsigned short iResponseType);
   ...
   *******************************************************************************/
   ...
   /******************************************************************************/
   /**
      API to get the Seuquencer results in the response packet (Sync response).

      \param iFTM_Error_Code, FTM_ERROR code returned by embedded side

      \param iRequestSize, Size of the request packet

      \param pRequestBytes, pointer to the request packet

      \param iResponseSize, size of the response packet

      \param pResponseBytes, pointer to the response packet

      \return true if the opreation is successful

      \warning QLIB_FTM_SEQ_ActivateBatchMode should be called first
   *******************************************************************************/
   QLIB_API unsigned char QLIB_FTM_SEQ_GET_COMPRESSED_MULTIPLE_COMMAND_RESPONSE_DATA
   (
      HANDLE hResourceContext, 
      unsigned short* iFTM_Error_Code,
      unsigned char iCompressionControl
   );
   /******************************************************************************/
   /**
      Executes the existing FTM Sequencer queue of FTM messages.
