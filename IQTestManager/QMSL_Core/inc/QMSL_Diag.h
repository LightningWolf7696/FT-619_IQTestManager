/******************************************************************************
 * $Header: //depot/HTE/QDART/QMSL6.1/QMSL_Core/inc/QMSL_Diag.h#10 $
...
extern "C" {
   ...
   *******************************************************************************/
   ...
   Returns the call status of the phone with specific system mode by calling
   the Call manager status command defined in 80-V1294-7
   
    First from CM_MULTISIM_STATE_INFO, check if support MultiSIM
   If not support MultiSim, call QLIB_GetPhoneCallState.
   If multi sim case, check every SIM, from CMLOG_STATE_INFO_PER_SUBS_F, find system mode match and return phone state.

   Note 1: This function is not available on AMSS which does not support the diagnostic subsystem
   commands CM_MULTISIM_STATE_INFO & CMLOG_STATE_INFO_PER_SUBS_F.
   ...
   \param piSystemMode = input & output, system mode
   \code
       SYS_SYS_MODE_CDMA    = 2, //!< ' Code Division Multiple Access (CDMA) mode
       SYS_SYS_MODE_GSM     = 3, //!< ' Global System for Mobile communications (GSM) mode       
       SYS_SYS_MODE_WCDMA   = 5, //!< ' Wideband CDMA (WCDMA) mode
       SYS_SYS_MODE_LTE     = 9,       //!< ' LTE
       SYS_SYS_MODE_TDS     = 11,    //!< ' GSM, WCDMA and LTE mode
   \endcode

   \return true if the function is successful

   *******************************************************************************/
   QLIB_API unsigned char QLIB_GetPhoneCallState_V3(
      HANDLE hResourceContext,
      unsigned long *piCallState,
      unsigned long *piSystemMode);
